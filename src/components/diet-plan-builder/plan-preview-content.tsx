    ...
    <div data-print-plan="true" className={cn("space-y-8 text-text-primary", className)}>
      ...
      <header className="space-y-4 border-b-2 border-neutral-200 pb-6 print:border-black">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <p className="text-[10px] font-extrabold uppercase tracking-[0.2em] text-text-muted print:text-neutral-600">Plano alimentar</p>
          ...
          <div className="text-right text-small12 font-semibold text-text-secondary print:text-neutral-800">
            {plan.professionalName.trim() ? (
              <>
                <p className="font-extrabold text-text-primary print:text-black">{plan.professionalName}</p>
        ...
        <div className="grid gap-3 rounded-xl bg-neutral-50/80 p-4 ring-1 ring-neutral-200/80 print:bg-white print:ring-black/20 sm:grid-cols-2">
          <div>
            <p className="text-[10px] font-extrabold uppercase tracking-wide text-text-muted">Paciente</p>
            <p className="mt-1 text-body16Semi text-text-primary print:text-black">
              {patientDisplayName.trim() || "—"}
            </p>
          </div>
          <div>
            <p className="text-[10px] font-extrabold uppercase tracking-wide text-text-muted">Documento</p>
        ...
        {plan.description.trim() ? (
          <div>
            <p className="text-[10px] font-extrabold uppercase tracking-wide text-text-muted">Descrição clínica</p>
        ...
        {plan.meals.map((meal, i) => (
          ...
          >
            <div className="flex flex-wrap items-baseline justify-between gap-2 border-b border-neutral-100 pb-3">
              <h4 className="text-title18 font-extrabold text-text-primary print:text-black">
                <span className="text-primary print:text-neutral-900">{i + 1}.</span> {meal.name.trim() || "Refeição"}
              </h4>
              <span className="rounded-full bg-primary/12 px-3 py-1 text-small12 font-extrabold text-text-primary ring-1 ring-primary/20 print:border print:border-neutral-400 print:bg-white">
                {meal.time || "—"}
              </span>
            </div>
            {meal.observation.trim() ? (
              <p className="mt-3 rounded-lg bg-yellow/10 px-3 py-2 text-small12 font-semibold leading-relaxed text-text-secondary print:bg-neutral-100">
                <span className="font-extrabold text-text-primary">Obs. da refeição: </span>
                {meal.observation.trim()}
              </p>
            ) : null}
            ...
            <div className="mt-5 space-y-6">
              {meal.groups.map((group) => (
                <div key={group.id}>
                  <p className="text-body14Semi font-extrabold uppercase tracking-wide text-secondary print:text-black">{group.name}</p>
                      ...
                      <li key={opt.id} className="text-body14">
                        ...
                        {opt.recipe.trim() ? (
                          <div className="mt-2 rounded-lg bg-neutral-50/90 p-3 text-small12 leading-relaxed text-text-secondary print:border print:border-neutral-300 print:bg-white">
                            <span className="font-extrabold text-text-primary print:text-black">Preparo: </span>
                            {opt.recipe.trim()}
                          </div>
                        ) : null}
                        {opt.imageUrl.trim() ? (
