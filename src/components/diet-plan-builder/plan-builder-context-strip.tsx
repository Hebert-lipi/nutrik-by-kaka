...
export function PlanBuilderContextStrip({
  ...
  const forWhom =
    plan.planKind === "patient_plan" && linked
      ? { label: "Paciente", name: linked.name, href: `/patients/${linked.id}` as const }
  ...
  return (
    ...
    >
      <div className="border-b border-neutral-100/90 bg-primary/[0.04] px-4 py-3 md:px-5">
        <p className="text-[11px] font-extrabold uppercase tracking-[0.12em] text-secondary">Contexto do plano</p>
        <div className="mt-2 flex flex-col gap-2 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between">
          <div className="min-w-0">
            ...
            {forWhom.href ? (
              <Link href={forWhom.href} className="mt-0.5 block truncate text-title16 font-extrabold text-text-primary underline-offset-4 hover:underline">
                {forWhom.name}
              </Link>
            ) : (
              <p className="mt-0.5 truncate text-title16 font-extrabold text-text-primary">{forWhom.name}</p>
            )}
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <Chip tone={plan.status === "published" ? "success" : "yellow"} className="font-extrabold">
      ...
      <div className="grid gap-4 px-4 py-4 sm:grid-cols-3 md:px-5">
        <div>
          <p className="text-[11px] font-extrabold uppercase tracking-wide text-text-muted">Versão atual</p>
          <p className="mt-1 text-body14 font-extrabold text-text-primary">v{plan.currentVersionNumber}</p>
        </div>
        <div className="sm:col-span-2">
          <p className="text-[11px] font-extrabold uppercase tracking-wide text-text-muted">Última atualização registrada</p>
