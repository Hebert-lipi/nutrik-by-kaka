    ...
    <div className={cn("space-y-8", className)}>
      ...
      {enabled && totalMeals > 0 ? (
        <div className="overflow-hidden rounded-2xl border border-primary/20 bg-gradient-to-br from-primary/[0.07] via-bg-0 to-neutral-50/40 p-4 shadow-premium-sm md:p-5">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <p className="text-body14 font-semibold text-text-primary">
              Você completou{" "}
            ...
            <span className="text-title16 font-semibold tabular-nums text-secondary">{progressPct}%</span>
          </div>
        ...
        MEAL_PERIOD_ORDER.map((period) => {
          ...
          return (
            <section key={period} className="space-y-4">
              <h2 className="text-title18 font-semibold tracking-tight text-text-primary">{mealPeriodLabel(period)}</h2>
                        ...
                        <div className="flex flex-wrap items-start justify-between gap-2">
                          <div>
                            <div className="flex flex-wrap items-center gap-2">
                              ...
                              {enabled ? (
                                <Chip tone={done ? "success" : "muted"} className="text-[10px] font-semibold uppercase">
                                  {done ? "Feito" : "Pendente"}
                                </Chip>
                              ) : null}
                            </div>
      ...
      {enabled ? (
        <Card className="border-neutral-200/60 shadow-premium-sm">
          <CardHeader className="border-b border-neutral-100/90 pb-3">
            <p className="text-title16 font-semibold text-text-primary">Observação do dia</p>
